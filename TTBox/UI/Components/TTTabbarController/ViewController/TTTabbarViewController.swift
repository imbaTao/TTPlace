...
class TTTabbarViewControllerItemModel: NSObject {
    ...
    // 初始化方法
    init(
        normalImageName: String, selectedImageName: String, itemContent: String,
        selected: Bool = false, isTuber: Bool = false
    ) {
        super.init()
    ...
    static func == (lhs: TTTabbarViewControllerItemModel, rhs: TTTabbarViewControllerItemModel)
        -> Bool
    {
...
class TTTabbarViewController: UITabBarController, TTTabbarViewControllerDelegate {
    ...
    func setupTabbar() {
        ...
        // rx绑定数据源,直接显示
        tabbarConfiguration.items.bind(
            to: htTabbar.bar.rx.items(cellIdentifier: "TTTabbarItem", cellType: TTTabbarItem.self)
        ) { [weak self] (collectionView, itemModel, cell) in
        ...
        htTabbar.bar.rx.itemSelected.subscribe(onNext: { [weak self] (indexPath) in
            ...
            if self?.canChangePage(index: indexPath.row) == true {
                ...
                if self?.lastClickItemModel != nil {
                    ...
                    if let lastIndex = tabbarConfiguration.sourceData.firstIndex(where: {
                        item -> Bool in
                        return item == self!.lastClickItemModel
                    }) {

                        let lastCell =
                            self!.htTabbar.bar.cellForItem(
                                at: IndexPath(row: lastIndex, section: 0)) as! TTTabbarItem
