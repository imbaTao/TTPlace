...
// 网格反选类都用这个类
class TTGridView: TTStackView, UICollectionViewDelegate, UICollectionViewDataSource,
    UICollectionViewDelegateFlowLayout
{
    ...
    // 渲染cell的时候回调
    var renderCellBlock: ((_ cell: TTCollectionViewCell, _ indexPath: IndexPath) -> Void)? {
        didSet {
            grid.reloadData()
        }
    }
    ...
    // 点击选中cell的回调
    var didSelectedCellBlock: ((_ cell: TTCollectionViewCell, _ indexPath: IndexPath) -> Void)?
    ...
    init(
        cellClassTypes: [TTCollectionViewCell.Type],
        _ configClosure: ((_ config: TTGridConfig) -> Void)?
    ) {
        ...
        -> UICollectionViewCell
    {
        let cell = collectionView.dequeueReusableCell(
            withClass: grid.cellClassTypes.first!, for: indexPath)
    ...
    func renderCell(_ outCell: UICollectionViewCell, _ indexPath: IndexPath) {
        if let cell = outCell as? TTCollectionViewCell {
            ...
            // 边框
            cell.borderColor =
                model.isSelected ? config.selectedBorderColor : config.unSelectedBorderColor
            cell.borderWidth = CGFloat(config.borderWidth)
            cell.backgroundColor =
                model.isSelected ? config.selectedBackGroundColor : config.unSelectedBackGroundColor

            if config.isEnabelSelectedColor {
                cell.mainLabel.textColor =
                    model.isSelected ? config.selectedTitleColor : config.unselectedTitleColor
                cell.subLabel.textColor =
                    model.isSelected ? config.selectedTitleColor : config.unselectedTitleColor
            }
