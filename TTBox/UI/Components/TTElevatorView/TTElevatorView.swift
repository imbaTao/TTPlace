...
class TTElevatorView<T: UIView>: UIControl {
    ...
    @discardableResult
    func creat(
        onWindow: Bool = true, parentView: UIView? = nil, contentView: T,
        contentViewConfigBlock: ((T) -> Void)? = nil, contentViewSize: CGSize,
        maskBackGroundColor: UIColor = rgba(0, 0, 0, 0.2), initShow: Bool = true,
        touchHidden: Bool = true
    ) -> Self {
    ...
    //MARK: - 隐藏动画
    func dismiss(complet: @escaping () -> Void) {
        self.animating = true
        UIView.animate(withDuration: animateInterval) {
            self.contentView.snp.updateConstraints { (make) in
