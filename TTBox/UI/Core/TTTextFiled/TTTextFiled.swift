...
class TTTextFiled: UITextField, UITextFieldDelegate {
    ...
    // 根据config初始化
    init(
        configure: TTTextFiledConfigure = TTTextFiledConfigure(),
        ...
        self.rx.text.changed.asObservable()
            .subscribe(onNext: { [weak self] _ in
                ...
                if let newText = self.text?.replacingOccurrences(of: " ", with: "") {
                    ...
                    // 大于10个，那么截取
                    if newText.lengthWhenCountingNonASCIICharacterAsTwo()
                        > self.configure.maxTextCount
                    {
                        let index = newText.index(
                            newText.startIndex, offsetBy: self.configure.maxTextCount)
    ...
    func textField(
        ...
        if let newText = textField.text?.appending(string).replacingOccurrences(of: " ", with: "") {
            //            debugPrint("新文本长度\(newText.lengthWhenCountingNonASCIICharacterAsTwo())")
            //            debugPrint("新文本\(newText)")
