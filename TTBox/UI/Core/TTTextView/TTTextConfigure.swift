...
class TTTextViewConfigure: NSObject {
    ...
    // 过滤器
    private(set) var filter: TTTextFilter? = nil
    var filterType = TTTextFilterType.initial {
        didSet {
            filter = .init(filterType)
        }
    }
}

enum TTTextFilterType: Equatable {
    case initial  // 默认空正则
    ...
    case ID  // 身份证
    case password  // 密码
    case apartEmoji  // 除了emoji以外都行
    case custom(rule: String) // 自定义
}
