...
class TTAuthorizer: NSObject {
    ...
    // MARK: - 检测是否开启定位
    /// 检测是否开启定位
    class func checkLocationServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
        var isOpen = false
        if CLLocationManager.authorizationStatus() != .restricted && CLLocationManager.authorizationStatus() != .denied {
            isOpen = true
        }
        if isOpen == false && isSet == true {tt_OpenURL(.location)}
        action(isOpen)
    }
}
    ...
    if authStatus == MPMediaLibraryAuthorizationStatus.notDetermined {
        MPMediaLibrary.requestAuthorization { (status) in
            if (status == MPMediaLibraryAuthorizationStatus.authorized) {
                ...
                DispatchQueue.main.async {
                    action(false)
                     if isSet == true {tt_OpenURL(.media)}
                }
            }
        }
...
func tt_openEventServiceWithBolck(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
    ...
    cellularData.cellularDataRestrictionDidUpdateNotifier = { (state) in
        if state == CTCellularDataRestrictedState.restrictedStateUnknown ||  state == CTCellularDataRestrictedState.notRestricted {
            action(false)
            if isSet == true {tt_OpenURL(.network)}
        } else {
            action(true)
        }
    }
...
func tt_openCaptureDeviceServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
    ...
    if authStatus == AVAuthorizationStatus.notDetermined {
        AVCaptureDevice.requestAccess(for: AVMediaType.video) { (granted) in
            action(granted)
            if granted == false && isSet == true {tt_OpenURL(.camera)}
        }
    } else if authStatus == AVAuthorizationStatus.restricted || authStatus == AVAuthorizationStatus.denied {
        action(false)
        if isSet == true {tt_OpenURL(.camera)}
    } else {
        action(true)
    }
}
...
func tt_openAlbumServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
    ...
    if authStatus == PHAuthorizationStatus.restricted || authStatus == PHAuthorizationStatus.denied {
        isOpen = false;
        if isSet == true {tt_OpenURL(.photo)}
    }
    action(isOpen)
}
...
func tt_openRecordServiceWithBlock(_ isSet:Bool? = nil,_ action :@escaping ((Bool)->())) {
    ...
    if permissionStatus == AVAudioSession.RecordPermission.undetermined {
        AVAudioSession.sharedInstance().requestRecordPermission { (granted) in
            action(granted)
            if granted == false && isSet == true {tt_OpenURL(.microphone)}
        }
        ...
        action(false)
        if isSet == true {tt_OpenURL(.microphone)}
    } else {
        action(true)
    }
}
// MARK: - 跳转系统设置界面
func tt_OpenURL(_ type: HWpermissionsType? = nil) {
    let title = "访问受限"
