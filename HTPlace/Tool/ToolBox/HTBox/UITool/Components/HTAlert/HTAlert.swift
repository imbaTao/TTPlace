...
// 点击隐藏视图
class HTTouchHiddenView: UIView {
...
// 如果需要点击其他区域进行隐藏或者改样式，就继承HTTouchHiddenView
class HTAlert: UIView {
    // 背板视图
...
//        self.mainContentTextView.setContentOffset(.zero, animated: false)
    ...
    //  创建时传入标题，内容，标题
    @discardableResult
    class func show(maxSize: CGSize,title: String,message: String,buttonTitles: [String], click:@escaping (_ index: Int)->()) -> HTAlert {
...
//              }).disposed(by: alert.rx.disposeBag)
    ...
    // 隐藏操作
    class func hiddenAction(alert: HTAlert) {
        alert.isHidden = true
...
extension UIView {
    ...
    // 这个是简单的内边框,外边框还没实现
    func addBorderWithPositon(direction: HTViewBorderDirection,color: UIColor,height: CGFloat) {
